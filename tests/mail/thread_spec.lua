...
describe('mail.thread', function()
  ...
  describe('build_cmd', function()
    it('builds nm-html-extract command from msgid', function()
      local cmd = thread.build_cmd('test-id@example.com')
      assert.is_truthy(cmd:find('nm-html-extract', 1, true))
      assert.is_truthy(cmd:find('test-id@example.com', 1, true))
    end)

    it('builds plain fallback command', function()
      local cmd = thread.build_cmd_plain('test-id@example.com')
      assert.is_truthy(cmd:find('notmuch', 1, true))
      assert.is_truthy(cmd:find('test-id@example.com', 1, true))
      assert.is_falsy(cmd:find('muttlook', 1, true))
    end)
  end)
end)
